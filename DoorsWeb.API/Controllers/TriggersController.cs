...
{
    // Plural class name ("TriggersController") avoids clashing with the TriggerController *entity*,
    // but the route is pinned to the singular "api/Trigger" to match the entity concept and the
    // existing client call sites (which already call api/Trigger).
    [Route("api/Trigger")]
    [ApiController]
    [Authorize(Policy = AreaPolicies.SiteSettingsRead)]
    public class TriggersController : ControllerBase
    {
        private readonly ITriggersService _service;

        public TriggersController(ITriggersService service)
        {
            _service = service;
        }
